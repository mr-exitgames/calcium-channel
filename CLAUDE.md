...
- The `calciumchannel.Mcp` dispatcher in the MCP VM reads `QREXEC_SERVICE_ARGUMENT`, looks up the server in `/rw/config/calcium-channel/registry.json`, loads env vars if specified, and execs the MCP server process.
- dom0 services (`McpList`, `McpRegister`) handle discovery and ACL management.
...
- The registry and env files live in `/rw/config/calcium-channel/` (persistent across AppVM reboots).
- `McpRegister` appends a default deny rule for each server — explicit allow entries must come first in the policy.
- The dom0 installer has the same stdin conflict as Salt Bridge — use the two-step install method.

## Development

- The git remote is `git@git:repos/calcium-channel.git` via SSH over `qubes.ConnectTCP` qrexec.
- This project is intentionally separate from Salt Bridge. Calcium Channel strengthens Qubes isolation; Salt Bridge bypasses it. Do not merge them.
